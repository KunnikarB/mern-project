...
interface Props {
  placeholder?: string;
  // optional callback if parent wants to handle selection
  onSelect?: (user: UserPreview) => void;
  // optional: override endpoint (default: /users?search=)
  endpoint?: string;
}
    ...
    <div ref={containerRef} className="relative w-full max-w-sm">
      <div className="relative">
        <input
          ...
          className="w-full rounded-xl border border-pinkyDark px-4 py-2 pr-12 focus:outline-none focus:ring-2 focus:ring-pinky-300"
        />
        ...
        <div className="absolute inset-y-0 right-2 flex items-center pointer-events-none">
          {loading ? (
            <svg
              className="w-5 h-5 animate-spin text-pinkyDark"
              viewBox="0 0 24 24"
              fill="none"
            >
              <circle
      ...
      {open && results.length > 0 && (
        <div className="absolute z-50 mt-2 w-full  border border-gray-200 rounded-lg shadow-lg overflow-hidden">
